...
debugmode=1
dumpdir=dump_taigi    # directory to dump full features
...
# exp tag
tag="taigi_baseline" # tag for managing experiments.
...
trans_set="taigi_test.zh taigi_train_dev.zh"
train_nj=8
...
dict=data/lang_1spm/${train_set}_units_${tgt_case}.txt
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    grep sp1.0 data/${train_set_prefix}.zh/text | cut -f 2- -d' ' | grep -v -e '^\s*$' > data/lang_1spm/input.txt
    ...
    data2json.sh --nj ${train_nj} --feat ${feat_tr_dir}/feats.scp --text data/${train_set}/text.${tgt_case} --bpecode ${bpemodel}.model --lang zh \
        data/${train_set} ${dict} > ${feat_tr_dir}/data.${src_case}_${tgt_case}.json
    data2json.sh --feat ${feat_dt_dir}/feats.scp --text data/${train_dev}/text.${tgt_case} --bpecode ${bpemodel}.model --lang zh \
    ...
    for x in ${train_set} ${trans_set}; do
        ...
        data_dir=data/$(echo ${x} | cut -f 1 -d ".").tai
