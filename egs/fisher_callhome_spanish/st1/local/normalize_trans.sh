...
for set in fisher_train fisher_dev fisher_dev2 fisher_test; do
    ...
    # split training data to 160/40/20 hours, respectively
    if [[ "${set}" == "fisher_train" ]]; then
      cp data/local/fisher-callhome-corpus/corpus/ldc/${set}.es data/${set}/es.joshua.org.tmp
      
      python local/fisher_split_train_data.py --type $3 --file all
      python local/fisher_split_train_data.py --type $3 --file es
      
      # keep original text file as reference
      mv data/fisher_train/text data/fisher_train_$3/text.org
      
      # clean up
      rm -rf data/fisher_train
      mv data/fisher_train_$3 data/fisher_train
    else
...
for set in fisher_train; do
    # having one En reference
    cp data/local/fisher-callhome-corpus/corpus/ldc/${set}.en data/${set}/en.org
    
    # split training data to 160/40/20 hours, respectively
    if [[ "${set}" == "fisher_train" ]]; then
      normalize-punctuation.perl -l en < data/${set}/en.org | sed -e "s/¿//g" | local/normalize_punctuation.pl > data/${set}/en.norm.tc.tmp
      python local/fisher_split_train_data.py --type $3 --file en
      rm data/${set}/en.norm.tc.tmp
    else
