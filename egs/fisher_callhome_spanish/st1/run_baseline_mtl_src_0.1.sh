...
debugmode=1
dumpdir=dump_sample_16k_bpe_1k_src_0.1   # directory to dump full features
...
train_config=train.yaml #conf/train.yaml
...
# exp tag
tag="baseline_mtl_src_0.1" # tag for managing experiments.
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for dataset in fisher_dev.es fisher_dev2.es fisher_test.es train_sp.es train_dev.es; do
        python local/phone_recognition.py \
          --phone-path data_sample_16k_bpe_1k_src_0.1/${dataset}/phone \
          --wav-scp-path data_sample_16k_bpe_1k_src_0.1/${dataset}/wav.scp \
          --segments data_sample_16k_bpe_1k_src_0.1/${dataset}/segments \
          --dataset ${dataset} \
          --phone-system ${phone_system} \
          --number-of-worker ${nj} > data_sample_16k_bpe_1k_src_0.1/${dataset}/phone.log
    done
    rm -rf ${phone_system}
fi

dict=data_sample_16k_bpe_1k_src_0.1/lang_1spm/${train_set}_${bpemode}${nbpe}_units_${tgt_case}.txt
phone_dict=data_sample_16k_bpe_1k_src_0.1/lang_1spm/phone.txt
phone_transform_dict=data_sample_16k_bpe_1k_src_0.1/lang_1spm/phone_map.txt
nlsyms=data_sample_16k_bpe_1k_src_0.1/lang_1spm/${train_set}_non_lang_syms_${tgt_case}.txt
bpemodel=data_sample_16k_bpe_1k_src_0.1/lang_1spm/${train_set}_${bpemode}${nbpe}_${tgt_case}
phone_bpemodel=data_sample_16k_bpe_1k_src_0.1/lang_1spm/${train_set}_${bpemode}${nbpe}_phone

echo "dictionary: ${dict}"
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    # Task dependent. You have to check non-linguistic symbols used in the corpus.
    echo "stage 2: Dictionary and Json Data Preparation"
    mkdir -p data_sample_16k_bpe_1k_src_0.1/lang_1spm/

    echo "make a non-linguistic symbol list for all languages"
    grep sp1.0 data_sample_16k_bpe_1k_src_0.1/${train_set_prefix}.*/text.${tgt_case} | cut -f 2- -d' ' | grep -o -P '&[^;]*;' | sort | uniq > ${nlsyms}
    cat ${nlsyms}
    ...
    grep sp1.0 data_sample_16k_bpe_1k_src_0.1/${train_set_prefix}.*/text.${tgt_case} | cut -f 2- -d' ' | grep -v -e '^\s*$' > data_sample_16k_bpe_1k_src_0.1/lang_1spm/input.txt
    spm_train --user_defined_symbols="$(tr "\n" "," < ${nlsyms})" --input=data_sample_16k_bpe_1k_src_0.1/lang_1spm/input.txt --vocab_size=${nbpe} --model_type=${bpemode} --model_prefix=${bpemodel} --input_sentence_size=100000000 --character_coverage=1.0
    spm_encode --model=${bpemodel}.model --output_format=piece < data_sample_16k_bpe_1k_src_0.1/lang_1spm/input.txt | tr ' ' '\n' | sort | uniq | awk -v offset=${offset} '{print $0 " " NR+offset}' >> ${dict}
    wc -l ${dict}
    ...
    # phone
    grep sp1.0 data_sample_16k_bpe_1k_src_0.1/${train_set_prefix}.es/phone | cut -f 2- -d' ' | grep -v -e '^\s*$' > data_sample_16k_bpe_1k_src_0.1/lang_1spm/phone_input.txt
    cat < data_sample_16k_bpe_1k_src_0.1/lang_1spm/phone_input.txt | tr ' ' '\n' | sort | uniq | awk -v offset=${phone_offset} '{print $0 " " NR+offset}' >> ${phone_dict}

    # phone bpe
    echo "<unk> 1" > ${phone_transform_dict}
    phone_transform_offset=$(wc -l < ${phone_transform_dict})

    python local/phone_mapping.py --phone-dict=${phone_dict} --phone-input=data_sample_16k_bpe_1k_src_0.1/lang_1spm/phone_input.txt >> data_sample_16k_bpe_1k_src_0.1/lang_1spm/phone_transform.txt

    spm_train --user_defined_symbols="$(tr "\n" "," < ${nlsyms})" --input=data_sample_16k_bpe_1k_src_0.1/lang_1spm/phone_transform.txt --vocab_size=${phone_nbpe} --model_type=${bpemode} --model_prefix=${phone_bpemodel} --input_sentence_size=100000000 --character_coverage=1.0
    spm_encode --model=${phone_bpemodel}.model --output_format=piece < data_sample_16k_bpe_1k_src_0.1/lang_1spm/phone_transform.txt | tr ' ' '\n' | sort | uniq | awk -v offset=${phone_transform_offset} '{print $0 " " NR+offset}' >> ${phone_transform_dict}

    echo "make json files"
    # only training set apply bpe-dropout
    data2json.sh --nj 16 --feat ${feat_tr_dir}/feats.scp --text data_sample_16k_bpe_1k_src_0.1/${train_set}/text.${tgt_case} --bpecode ${bpemodel}.model --bpe_dropout ${bpe_dropout} --lang en \
        data_sample_16k_bpe_1k_src_0.1/${train_set} ${dict} > ${feat_tr_dir}/data_${bpemode}${nbpe}.${src_case}_${tgt_case}.json
    data2json.sh --feat ${feat_dt_dir}/feats.scp --text data_sample_16k_bpe_1k_src_0.1/${train_dev}/text.${tgt_case} --bpecode ${bpemodel}.model --lang en \
        data_sample_16k_bpe_1k_src_0.1/${train_dev} ${dict} > ${feat_dt_dir}/data_${bpemode}${nbpe}.${src_case}_${tgt_case}.json
    for ttask in ${trans_set}; do
        feat_trans_dir=${dumpdir}/${ttask}/delta${do_delta}
        data2json.sh --feat ${feat_trans_dir}/feats.scp --text data_sample_16k_bpe_1k_src_0.1/${ttask}/text.${tgt_case} --bpecode ${bpemodel}.model --lang en \
            data_sample_16k_bpe_1k_src_0.1/${ttask} ${dict} > ${feat_trans_dir}/data_${bpemode}${nbpe}.${src_case}_${tgt_case}.json
    done
    ...
    for ttask in fisher_dev.en fisher_dev2.en fisher_test.en; do
        feat_trans_dir=${dumpdir}/${ttask}/delta${do_delta}
        for no in 1 2 3; do
            data2json.sh --text data_sample_16k_bpe_1k_src_0.1/${ttask}/text.${tgt_case}.${no} --feat ${feat_trans_dir}/feats.scp --bpecode ${bpemodel}.model --lang en \
                data_sample_16k_bpe_1k_src_0.1/${ttask} ${dict} > ${feat_trans_dir}/data_${bpemode}${nbpe}_${no}.${tgt_case}.json
        done
    done

    # phone bpe encode for all of different dataset
    for x in ${train_dev} fisher_dev.en fisher_dev2.en fisher_test.en; do
        data_dir=data_sample_16k_bpe_1k_src_0.1/$(echo ${x} | cut -f 1 -d ".").es
    ...
    train_data_dir=data_sample_16k_bpe_1k_src_0.1/$(echo ${train_set} | cut -f 1 -d ".").es
