...
Usage: $0 <json> <data-dir> <dict> <trans-type>
e.g. $0 data/train data/lang_1char/train_units.txt "phn"
Options:
...
if [ -n "${bpecode}" ]; then
    ...
    text2token.py -s 1 -n 1 -l ${nlsyms} -t ${trans_type} ${text} > ${tmpdir}/output/token.scp
else
    
    text2token.py -s 1 -n 1 -t ${trans_type} ${text} > ${tmpdir}/output/token.scp
fi
