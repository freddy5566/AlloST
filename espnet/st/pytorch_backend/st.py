...
class CustomConverter(ASRCustomConverter):
    ...
    Args:
        ...
        use_source_phone (bool): use source phone sequence

    """

    def __init__(
        self, subsampling_factor=1, dtype=torch.float32, use_source_text=False, use_phone_sequence=False,
    ):
        """Construct a CustomConverter object."""
        super().__init__(subsampling_factor=subsampling_factor, dtype=dtype)
        self.use_source_text = use_source_text
        self.use_phone_sequence = use_phone_sequence
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        assert len(batch) == 1
        # use phone location to test
        if self.use_phone_sequence:
            xs, ys, ys_src, ys_phn = batch[0]
        else:
            xs, ys, ys_src, ys_phn = batch[0]
        ...
        if self.use_phone_sequence:
            phone_lens = np.array([y.shape[0] for y in ys_phn])
            phone_lens = torch.from_numpy(phone_lens).to(device)

            ys_pad_phn = pad_list(
                [torch.from_numpy(np.array(y, dtype=np.int64)) for y in ys_phn],
                pad_value=0,
            ).to(device)
        else:
            ys_pad_phn = None
        ...
        if self.use_phone_sequence:
            return xs_pad, ilens, ys_pad, ys_pad_src, ys_pad_phn, phone_lens
        else:
            return xs_pad, ilens, ys_pad, ys_phn
...
def train(args):
    ...
    if args.enc_init is not None or args.dec_init is not None:
        ...
        if args.model_module == "espnet.nets.pytorch_backend.e2e_st_dual_encoder_conformer:E2E":
            phone_dim = int(valid_json[utts[0]]["output"][2]["shape"][-1])
            args.phone_dim = phone_dim
    ...
    converter = CustomConverter(
        ...
        use_phone_sequence=True if args.model_module == "espnet.nets.pytorch_backend.e2e_st_dual_encoder_conformer:E2E" else False,
    )
...
def trans(args):
    ...
    if args.batchsize == 0:
        with torch.no_grad():
            # TODO(Freddy): currently hard core to build the feature
            for idx, name in enumerate(js.keys(), 1):
                logging.info("(%d/%d) decoding " + name, idx, len(js.keys()))
                batch = [(name, js[name])]
                inputs_and_targets = load_inputs_and_targets(batch)

                feat = inputs_and_targets[0][0]

                # currently only support dual encoder conformer model
                # TODO(Freddy): make original model work again
                phone_sequence = torch.tensor(inputs_and_targets[3], dtype=torch.int64)
