...
class DualEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        if pos_enc_layer_type == "abs_pos":
            ...
            raise ValueError("unknown pos_enc_layer: " + args.transformer_encoder_pos_enc_layer_type)
        ...
        self.auxiliary_embed = torch.nn.Sequential(
            torch.nn.Embedding(auxiliary_idim, attention_dim, padding_idx=auxiliary_padding_idx),
            pos_enc_class(attention_dim, positional_dropout_rate),
        )
    ...
    def forward(self, xs, masks, phone, phone_mask):
        ...
        if self.is_share_weights:
            self.auxiliary_attns = [encoder.self_attn.attn for encoder in auxiliary_encoders]
