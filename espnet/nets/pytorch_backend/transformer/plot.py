...
def plot_multi_head_attention(
    ...
    for name, att_ws in attn_dict.items():
        for idx, att_w in enumerate(att_ws):
            ...
            if "auxiliary_encoders" in name:
                phone_len = int(data[idx][1][okey][2]["shape"][0])
                att_w = att_w[:, : phone_len, : phone_len]
                
                xtokens = data[idx][1][okey][2]["token"].split()
                ytokens = data[idx][1][okey][2]["token"].split()
                ...
                if is_mt:
                    ...
                    ytokens = xtokens[:]
            elif "fusion_attn" in name:
                phone_len = int(data[idx][1][okey][2]["shape"][0])
                att_w = att_w[:, : enc_len, : phone_len]
                xtokens = data[idx][1][okey][2]["token"].split()
                ...
                elif "auxiliary_src_attn" in name:
                    phone_len = int(data[idx][1][okey][2]["shape"][0])
                    att_w = att_w[:, :dec_len, :phone_len]
